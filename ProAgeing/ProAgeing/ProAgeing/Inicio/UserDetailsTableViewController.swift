...
class UserDetailsTableViewController: UITableViewController {
    ...
    */
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
       ...
       // self.showMessage(text: seleccion)
        self.updateUser(campo: indexPath.row)
    ...
    */
    ...
    func updateUser(campo: Int) {
       
        let alert = UIAlertController(title: "Editar " + self.titulo[campo], message: "Escriba nuevo valor:", preferredStyle: .alert)
        

        alert.addTextField { (textField) in
            textField.text = self.userData[campo]
        }
        ...
        alert.addAction(UIAlertAction(title: "OK", style: .default, handler: { [weak alert] (_) in
            ...
            if self.appDelegate.usuario.updateUser(propiedad: self.titulo[campo], valor: (textField?.text!)!) {
                self.userData[campo] = (textField?.text!)!
                self.tableView.reloadData()
            } else {
                self.showMessage(text: "Este valor no se puede editar.")
            }
    ...
    func showMessage(text: String) {
        ...
        _ = Timer.scheduledTimer(timeInterval: 2.0, target: self, selector: #selector(AllContactsTableViewController.dismissAlert), userInfo: nil, repeats: true)
